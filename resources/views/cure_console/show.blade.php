...
<div class="container py-5">
    <div class="d-flex justify-content-between align-items-center mb-4">
        ...
        <div class="d-flex gap-2 flex-wrap">
            <a href="{{ route('console.cure.edit', $record->id) }}" class="btn btn-primary btn-sm">Edit</a>
            <form action="{{ route('console.cure.destroy', $record->id) }}" method="POST" class="d-inline" onsubmit="return confirm('Delete this cure upload?');">
                @csrf
                @method('DELETE')
                <button type="submit" class="btn btn-danger btn-sm">Delete</button>
            </form>
            <a href="{{ route('console.cure.download', $record->id) }}" class="btn btn-outline-primary btn-sm">
