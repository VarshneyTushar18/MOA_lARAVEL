...
script_path = os.path.join(base, "_deploy_hero_carousel.sh")
...
r1 = subprocess.run(
    ["scp", "-i", key, "-o", "StrictHostKeyChecking=accept-new", script_path, f"{host}:{remote_up}/deploy_hero_carousel.sh"],
    capture_output=True,
    text=True,
    timeout=180,
)
print("scp script", r1.returncode)
if r1.returncode != 0:
    raise SystemExit(1)

r2 = subprocess.run(
    ["ssh", "-i", key, "-o", "StrictHostKeyChecking=accept-new", host, f"bash {remote_up}/deploy_hero_carousel.sh"],
    capture_output=True,
    text=True,
    timeout=180,
)
