"""Upload TB Pledge videos (section 53) — all MP4s with live progress log."""
import argparse
import os
import re
import subprocess
...
SECTION_ID = 53
FOLDER = r"F:\1. LTBI WEBSITE FOLDERS\5. ACSM and IEC\8. Tb Pledge and Awareness Videos\2. All Videos_2024"
VIDEO_EXTS = {".mp4", ".mov", ".avi", ".webm", ".mkv"}
LOG = os.path.join(os.path.dirname(__file__), "_upload_tb_pledge_videos.log")
STATUS = os.path.join(os.path.dirname(__file__), "_upload_tb_pledge_videos_status.txt")
...
"""
    local_php = os.path.join(os.path.dirname(__file__), "_attach_tb_pledge_one.php")
    with open(local_php, "w", encoding="utf-8", newline="\n") as f:
        f.write(php)
    subprocess.run(
        ["scp", "-i", KEY, "-o", "StrictHostKeyChecking=accept-new", local_php, f"{HOST}:{APP}/_attach_tb_pledge_one.php"],
        check=True,
    )
    return subprocess.run(
        ["ssh", "-i", KEY, HOST, f"cd {APP} && php82 _attach_tb_pledge_one.php"],
        capture_output=True,
        text=True,
    )
...
def main() -> None:
    ...
    log(f"TB Pledge section {SECTION_ID} — {len(files)} videos, {total_mb:.1f} MB ({total_mb/1024:.2f} GB) total")
    ...
    log("ALL DONE — TB Pledge videos uploaded.")
