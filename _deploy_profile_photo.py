...
script_path = os.path.join(base, "_deploy_profile_photo.sh")
...
subprocess.run(
    ["scp", "-i", key, "-o", "StrictHostKeyChecking=accept-new", script_path, f"{host}:{remote_up}/deploy_profile_photo.sh"],
    check=True,
)
r2 = subprocess.run(
    ["ssh", "-i", key, host, f"bash {remote_up}/deploy_profile_photo.sh"],
    capture_output=True,
    text=True,
    timeout=180,
)
