...
rm -f "$PUB/assets/images/aiia-header-logo.png" "$APP/public/assets/images/aiia-header-logo.png"
cd "$APP"
php82 artisan view:clear
php82 -r "if (function_exists('opcache_reset')) opcache_reset();"
echo DONE
grep -n "aiia-header" "$APP/resources/views/layout/frontend.blade.php"
"""

script_path = os.path.join(base, "_deploy_aiia_live.sh")
...
subprocess.run(["scp", "-i", key, "-o", "StrictHostKeyChecking=accept-new", script_path, f"{host}:{remote_up}/deploy_aiia_live.sh"], check=True)
r2 = subprocess.run(["ssh", "-i", key, host, f"bash {remote_up}/deploy_aiia_live.sh"], capture_output=True, text=True, timeout=180)
print(r2.stdout)
print(r2.stderr)
