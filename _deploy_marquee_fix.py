...
script_path = os.path.join(base, "_deploy_marquee_fix.sh")
...
subprocess.run(["scp", "-i", key, "-o", "StrictHostKeyChecking=accept-new", script_path, f"{host}:{remote_up}/deploy_marquee_fix.sh"], check=True)
r2 = subprocess.run(["ssh", "-i", key, "-o", "StrictHostKeyChecking=accept-new", host, f"bash {remote_up}/deploy_marquee_fix.sh"], capture_output=True, text=True, timeout=180)
print(r2.stdout)
print(r2.stderr)
