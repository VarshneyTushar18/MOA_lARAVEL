...
script_path = os.path.join(base, "_deploy_video_limits.sh")
...
subprocess.run(
    ["scp", "-i", KEY, "-o", "StrictHostKeyChecking=accept-new", script_path, f"{HOST}:{REMOTE_UP}/deploy_video_limits.sh"],
    check=True,
)
r2 = subprocess.run(
    ["ssh", "-i", KEY, HOST, f"bash {REMOTE_UP}/deploy_video_limits.sh"],
    capture_output=True,
    text=True,
    timeout=120000,
)
