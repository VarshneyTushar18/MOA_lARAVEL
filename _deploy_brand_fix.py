...
cp "$UP/aiia-header-brand.png" "$APP/public/assets/images/aiia-header-logo.png"
cp "$UP/aiia-header-brand.png" "$PUB/assets/images/aiia-header-logo.png"
cd "$APP"
...
script_path = os.path.join(base, "_deploy_brand_fix.sh")
...
subprocess.run(["scp", "-i", key, "-o", "StrictHostKeyChecking=accept-new", script_path, f"{host}:{remote_up}/deploy_brand_fix.sh"], check=True)
r2 = subprocess.run(["ssh", "-i", key, host, f"bash {remote_up}/deploy_brand_fix.sh"], capture_output=True, text=True, timeout=180)
print(r2.stdout)
print(r2.stderr)
