...
script = f"""#!/bin/bash
set -e
cp {REMOTE_UP}/factsheet.blade.php {APP}/resources/views/pages/factsheet.blade.php
...
script_path = os.path.join(base, "_deploy_workshop_video_carousel.sh")
...
subprocess.run(
    ["scp", "-i", KEY, "-o", "StrictHostKeyChecking=accept-new", script_path, f"{HOST}:{REMOTE_UP}/deploy_workshop_video_carousel.sh"],
    check=True,
)
r2 = subprocess.run(
    ["ssh", "-i", KEY, HOST, f"bash {REMOTE_UP}/deploy_workshop_video_carousel.sh"],
    capture_output=True,
    text=True,
    timeout=120000,
)
