...
PUB="{pub}"
UP="{remote_up}"
mkdir -p "$APP/resources/views/pages" "$APP/resources/views/layout" "$APP/public/assets/css" "$PUB/assets/css"
...
script_path = os.path.join(base, "_deploy_video_music.sh")
...
subprocess.run(["scp", "-i", key, "-o", "StrictHostKeyChecking=accept-new", script_path, f"{host}:{remote_up}/deploy_video_music.sh"], check=True)
r2 = subprocess.run(["ssh", "-i", key, host, f"bash {remote_up}/deploy_video_music.sh"], capture_output=True, text=True, timeout=180)
print(r2.stdout)
print(r2.stderr)
