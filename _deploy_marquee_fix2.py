...
php82 -r "if (function_exists('opcache_reset')) {{ opcache_reset(); echo 'opcache cleared'; }} else {{ echo 'no opcache'; }}"
echo DONE
"""

script_path = os.path.join(base, "_deploy_marquee_fix2.sh")
...
subprocess.run(["scp", "-i", key, "-o", "StrictHostKeyChecking=accept-new", script_path, f"{host}:{remote_up}/deploy_marquee_fix2.sh"], check=True)
r2 = subprocess.run(["ssh", "-i", key, host, f"bash {remote_up}/deploy_marquee_fix2.sh"], capture_output=True, text=True, timeout=180)
print(r2.stdout)
print(r2.stderr)
