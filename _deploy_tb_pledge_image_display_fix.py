...
with open(os.path.join(base, "_deploy_tb_pledge_image_display_fix.sh"), "w", encoding="utf-8", newline="\n") as f:
    f.write(script)
...
subprocess.run(
    ["scp", "-i", KEY, os.path.join(base, "_deploy_tb_pledge_image_display_fix.sh"), f"{HOST}:{REMOTE_UP}/deploy_tb_pledge_image_display_fix.sh"],
    check=True,
)
r2 = subprocess.run(
    ["ssh", "-i", KEY, HOST, f"bash {REMOTE_UP}/deploy_tb_pledge_image_display_fix.sh"],
