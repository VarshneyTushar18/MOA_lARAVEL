...
uploads = [
    ...
    ("resources/views/pages/rntcp.blade.php", "rntcp.blade.php"),
...
cp "$UP/rntcp.blade.php" "$APP/resources/views/pages/rntcp.blade.php"
...
script_path = os.path.join(base, "_deploy_rntcp_page.sh")
...
subprocess.run(["scp", "-i", key, "-o", "StrictHostKeyChecking=accept-new", script_path, f"{host}:{remote_up}/deploy_rntcp_page.sh"], check=True)
r2 = subprocess.run(["ssh", "-i", key, "-o", "StrictHostKeyChecking=accept-new", host, f"bash {remote_up}/deploy_rntcp_page.sh"], capture_output=True, text=True, timeout=180)
print(r2.stdout)
print(r2.stderr)
