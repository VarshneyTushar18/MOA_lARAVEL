...
script_path = os.path.join(base, "_deploy_workshop_carousel.sh")
...
subprocess.run(
    ["scp", "-i", KEY, "-o", "StrictHostKeyChecking=accept-new", script_path, f"{HOST}:{REMOTE_UP}/deploy_workshop_carousel.sh"],
    check=True,
)
r2 = subprocess.run(
    ["ssh", "-i", KEY, HOST, f"bash {REMOTE_UP}/deploy_workshop_carousel.sh"],
    capture_output=True,
    text=True,
    timeout=120000,
)
