...
pairs = [
    ("app/Http/Controllers/Concerns/FiltersConsoleDateRange.php", "FiltersConsoleDateRange.php"),
    ...
    ("resources/views/partials/console_date_filter.blade.php", "console_date_filter.blade.php"),
...
script_path = os.path.join(base, "_deploy_date_range.sh")
...
subprocess.run(
    ["scp", "-i", key, "-o", "StrictHostKeyChecking=accept-new", script_path, f"{host}:{remote_up}/deploy_date_range.sh"],
    check=True,
)
r2 = subprocess.run(
    ["ssh", "-i", key, host, f"bash {remote_up}/deploy_date_range.sh"],
    capture_output=True,
    text=True,
    timeout=300000,
)
