...
pairs = [
    ...
    ("resources/views/layout/frontend.blade.php", "frontend_layout.blade.php"),
]
...
cp {remote_up}/frontend_layout.blade.php {app}/resources/views/layout/frontend.blade.php
cd {app}
...
script_path = os.path.join(base, "_deploy_roles_cards.sh")
...
subprocess.run(
    ["scp", "-i", key, "-o", "StrictHostKeyChecking=accept-new", script_path, f"{host}:{remote_up}/deploy_roles_cards.sh"],
    check=True,
)
r2 = subprocess.run(
    ["ssh", "-i", key, host, f"bash {remote_up}/deploy_roles_cards.sh"],
    capture_output=True,
    text=True,
    timeout=120000,
)
